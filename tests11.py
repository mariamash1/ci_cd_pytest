...
def test_row_count(db_connection):
    ...
    assert result[0] == 25, "Should return row count 25 for hr.countries table"
...
def test_null_values(db_connection):
    ...
    cursor.execute("with cte as (SELECT country_id, country_name, region_id "
                                           ...
                                           "END AS res FROM cte")
    result = cursor.fetchone()
    assert result[0] == 1, "Should return 1 for hr.countries table if there is no NULL values"
...
def test_duplicates(db_connection):
    ...
    result = cursor.fetchone()
    assert result[0] == 1, "Should return 1 for hr.jobs table if there is no duplicates"
...
def test_min_max_salary_comparison(db_connection):
    ...
    result = cursor.fetchone()
    assert result[0] == 1, "Should return 1 for hr.jobs table if min salary is not more than maximal salary"
...
def test_email(db_connection):
    ...
    result = cursor.fetchone()
    assert result[0] == 1, "Should return 1 if email field follows specified format in hr.employees table"
...
def test_main_manager_salary(db_connection):
    ...
    result = cursor.fetchone()
    assert result[0] == 1, "Should return 1 if main manager has the biggest salary in hr.employees table"
